<template>
  <div class="flex flex-col w-full mx-auto max-w-lg max-md:mt-10 lg:max-w-full justify-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-8 mt-8">
      <TicDriveRadio v-for="service in services" :key="service.id" :id="service.id" :name="service.title"
        :value="service" :isChecked="!!stepStore.stepFourData.services.find(
          s => s.id === service.id
        )
...
const services: Service[] = [
  ...
  },
  {
    ...
    description: 'Controllo completo del veicolo per garantire la sicurezza e conformità',
    icon: 'car-check'
  },
  {
    id: 4,
  ...
  },
  {
    ...
    description: 'Trattamento estetico per ripristinare la brillantezza della vernice',
    icon: 'car-polish'
  },
];
</script>
