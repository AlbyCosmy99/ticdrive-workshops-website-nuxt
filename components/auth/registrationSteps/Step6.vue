<template>
  ...
  >
    ...
    <h1 class="text-gray-500 text-2xl font-semibold mt-10">Foto dell'Autofficina</h1>    
    <h1 class="text-gray-500 text-lg font-semibold mt-3">Carica fino a 5 foto dell'autofficina per mostrarle ai clienti.</h1>    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-5 mb-10">
      <div 
        ...
        @click="openImage(-1)"
      >
        <div v-if="!images[4]" class="transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 absolute">
          <img src="/images/add_photo.jpg" class="w-5 h-5 mx-auto"/>
          <h1 class="text-gray-500 text-md font-normal mt-2">Upload photo</h1>
          <input type="file" accept="image/*" ref="mainInputRefs" class="hidden" @change="handleImage($event, 4)"/>
        </div>
        <div v-else class="relative w-full h-full">
          <img :src="images[4]" class="w-full h-full"/>
          <img src="/images/image-delete.png" class="w-10 h-10 absolute top-2 right-2 cursor-pointer" @click.stop.prevent="removeImage(4)" />
        </div>
      </div>
      <div 
        class="grid grid-cols sm:grid-cols-2 gap-2 rounded-xl"
      >
        <div 
        ...
        >
          ...
          <div v-else class="relative w-full h-full">
            ...
            <img src="/images/image-delete.png" class="w-6 h-6 absolute top-2 right-2 cursor-pointer" @click.stop.prevent="removeImage(index)" />
          </div>
        </div>
      </div>
    </div>
