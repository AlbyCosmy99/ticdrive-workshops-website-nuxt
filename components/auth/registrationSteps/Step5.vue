  ...
  >
    ...
    >
      <CheckBox
        ...
        :value="value.value"
        :isCheck="
          props.stepValues.currentServiceDays.some(
            day => day.value === value.value,
          )
        "
      ...
      >
        ...
        <div
          v-if="
            timeSlots[value.value][1]['start'] &&
            timeSlots[value.value][1]['end']
          "
...
const timeSlots = computed(() => {
  ...
  serviceDays.forEach(day => {
    const dayServiceTimes = props.stepValues.currentServiceDays.find(
      (d: any) => d.value === day.value,
    );

    if (dayServiceTimes) {
      slots[day.value] = [
        {
          start: stringToDate(dayServiceTimes.serviceTime1.start),
          end: stringToDate(dayServiceTimes.serviceTime1.end),
        },
        {
          start: stringToDate(dayServiceTimes.serviceTime2.start),
          end: stringToDate(dayServiceTimes.serviceTime2.end),
        },
      ];
    } else {
      slots[day.value] = [
...
const updateTime = (dayValue: number) => {
  const index = props.stepValues.currentServiceDays.findIndex(
    (d: any) => d.value === dayValue,
  );
  useStore.step_four_value.currentServiceDays[index].serviceTime1.start =
    dateToString(timeSlots.value[dayValue][0]['start']);
  useStore.step_four_value.currentServiceDays[index].serviceTime1.end =
    dateToString(timeSlots.value[dayValue][0]['end']);
  useStore.step_four_value.currentServiceDays[index].serviceTime2.start =
    dateToString(timeSlots.value[dayValue][1]['start']);
  useStore.step_four_value.currentServiceDays[index].serviceTime2.end =
    dateToString(timeSlots.value[dayValue][1]['end']);
};
const updateMaxVehicle = value => {
  if (value === 'minus') {
    ...
    console.log(value);
    useStore.step_four_value.maxVehicleNumber =
      useStore.step_four_value.maxVehicleNumber - 1;
  } else {
    useStore.step_four_value.maxVehicleNumber =
      useStore.step_four_value.maxVehicleNumber + 1;
  }
