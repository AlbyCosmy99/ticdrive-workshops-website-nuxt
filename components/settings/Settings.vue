        ...
        <div class="flex items-center gap-4 justify-between">
          <div class="flex items-center gap-4">
            <div v-if="authStore.user?.images.length" class="rounded-full overflow-hidden h-[94px] w-[94px]">
              <NuxtImg :src="authStore.user?.images[0].url" alt="Profile Image" class="object-cover w-full h-full" />
            </div>
            <div>
          ...
          <div class="flex gap-2">
            <TicDrivebutton v-if="isEditing" label="Annulla" @click="cancelEdit"
              custom-class="text-sm font-bold rounded-lg pl-2.5 pr-2.5 pt-2.5 pb-2.5 w-[100px] bg-red-500 hover:bg-red-600 text-white" />
            <TicDrivebutton :label="isEditing ? (loading ? 'Salvando...' : 'Salva') : 'Modifica'
              " @click="onEditProfile" :disabled="loading"
      ...
      <div class="px-6">
        ...
        <div class="mb-6 border-b pb-6">
          ...
          <div v-else class="w-[400px]">
            <TicDriveInput v-model="userData.name" placeholder="Inserisci il tuo nome completo" size="small" />
          </div>
        </div>
        ...
        <div class="mb-6 border-b pb-6">
          ...
          <div v-else class="w-[400px]">
            <TicDriveInput v-model="userData.email" placeholder="Inserisci la tua email" type="email" size="small" />
          </div>
        </div>
        ...
        <div class="mb-6 border-b pb-6">
          ...
          <div v-else class="w-[400px]">
            <TicDriveInput v-model="userData.phoneNumber" placeholder="Inserisci il tuo numero di telefono" type="tel"
              size="small" />
          </div>
        </div>
        ...
        <div class="mb-6 border-b pb-6">
          ...
          <div v-else class="flex flex-col w-[400px] gap-2">
            <TicDriveInput v-model="userData.address.streetAddress" placeholder="Via e Numero Civico" size="small" />
            <TicDriveInput v-model="userData.address.city" placeholder="Città" size="small" />
            <TicDriveInput v-model="userData.address.province" placeholder="Provincia" size="small" />
            <TicDriveInput v-model="userData.address.postalCode" placeholder="CAP" size="small" />
          </div>
        </div>

        <div class="mb-6 border-b pb-6">
          <button @click="changePassword" class="text-drive text-base font-normal hover:text-green-700">
            Cambio Password
          </button>
        </div>

        <div class="mb-6 border-b pb-6">
          <button @click="authStore.logout()" class="text-red-500 text-base font-normal hover:text-red-700">
            Logout
          </button>
        </div>
      </div>
