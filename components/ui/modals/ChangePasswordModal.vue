...
const handleSubmit = async () => {
  if (passwordMismatch.value)
    return showToast(
      ...
      'Aggiusta le password.',
    );
  if (!currentPassword.value)
    return showToast('warn', 'Password corrente obbligatoria.', 'Inseriscila.');
  if (!newPassword.value)
    return showToast('warn', 'Nuova password obbligatoria.', 'Inseriscila.');
  if (!confirmPassword.value)
    return showToast('warn', 'Conferma obbligatoria.', 'Ripeti la password.');
