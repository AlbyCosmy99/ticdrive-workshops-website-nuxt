...
<template>
    <tr>
      ...
      <td class="px-6 py-4 whitespace-nowrap">
        <div :class="['px-3 py-1 rounded-md inline-flex items-center', ratingBackgroundColor]">
          <div class="flex">
            <template v-for="i in 5" :key="i">
              <img 
                ...
                :class="[
                  'w-4 h-4', 
                  i <= parseInt(rating) ? 'text-amber-500' : 'text-amber-300 opacity-40'
                ]"
              />
              <span v-else :class="['text-amber-500', i > parseInt(rating) ? 'opacity-40' : '']">
                {{ i <= parseInt(rating) ? '★' : '☆' }}
              </span>
            </template>
          </div>
